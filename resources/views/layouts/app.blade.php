...
<body class="min-h-screen bg-background text-foreground antialiased">
    ...
    <script>
        ...
        // Simpan pesan flash ke dalam objek JavaScript
        const flashMessages = {
            success: "{{ session('success') }}",
            newsletter_success: "{{ session('newsletter_success') }}",
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (flashMessages.success) {
                setTimeout(() => {
                    window.DLH.utils.showNotification(flashMessages.success, 'success');
                }, 500);
            }
            if (flashMessages.newsletter_success) {
                setTimeout(() => {
                    window.DLH.utils.showNotification(flashMessages.newsletter_success, 'success');
                }, 500);
            }
            if (flashMessages.error) {
                setTimeout(() => {
                    window.DLH.utils.showNotification(flashMessages.error, 'error');
                }, 500);
            }
        });
    </script>
